...
async function validatePostForm(form, formValues) {
  try {
    ...
    //abortEarly: false có bao nhiêu lỗi validate hết
    ...
    if (error.name === 'ValidationError' && Array.isArray(error.inner)) {
      for (const validationError of error.inner) {
        ...
        // bỏ qua nếu trường validate đã có
...
function initUploadImage(form) {
  // prev upload
  // mỗi lần thay đổi file dưới local -> use URL.createObjectURL(input.files[0])
...
export default function initPostForm({ formId, defaultValues, onSubmit }) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // truyền form lên tk cha phải gắn id vào
